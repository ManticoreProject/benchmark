...
structure BarnesHut : sig

    type scalar = double

    datatype particle = P of scalar * scalar * scalar * scalar * scalar

    val one_step : particle list -> particle list
...
structure Main =
  struct
    ...
    val dfltN = 400000   (* default number of bodies *)
    ...
    fun particle (mass, (xp, yp), (xv, yv)) = BH.P (xp, yp, mass, xv, yv)
    fun genBodies n = List.map particle (GenBodies.testdata n)

    val epsilon = 0.0000000001
    fun bumpParticle (BH.P (xp, yp, mass, xv, yv)) =
	BH.P (xp+epsilon, yp+epsilon, mass+epsilon, xv+epsilon, yv+epsilon)
	...
	let
	     ...
	     *)
	    val bodiesArray = RunSeq.runSilent (fn _ => map bumpParticle bodiesList)
	    fun doit () =
		let
		    fun iter (ps, i) =
			if i < nIters then
			    iter (BH.one_step ps, i + 1)
			else
			    ps
