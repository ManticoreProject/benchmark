structure IdRaytracer = struct
   ...
   * version.
   *)
  fun zerovector ((x,y,z) : vec) = (x < EPSILON andalso y < EPSILON andalso z < EPSILON);
...
(* THE NEXT TWO LINES CAUSE SEG FAULT *)
(*
      val side = [| 0 to (winsize-1) |]
      val scene = [| [| f (i, j) | j in side |] | i in side |]
*)
      val tab = Rope.tabulate
      val scene = tab (winsize, fn i => tab (winsize, fn j => f (i, j)))
