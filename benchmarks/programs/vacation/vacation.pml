...
val mainSeed = R.newSeed 0
val _ = R.inRangeInt(0, 100, mainSeed)

fun shuffle(r, n, i) = 
    if i = 0
    then r
    else let val x = R.inRangeInt(0, n, mainSeed)
             val y = R.inRangeInt(0, n, mainSeed)
             val temp = UA.sub(r, x)
...
fun populate(tree, ids, i, add) = 
    ...
    then ()
             ...
             val num = R.inRangeInt(100, 501, mainSeed)
             val price = R.inRangeInt(50, 91, mainSeed)
...
fun start i =
    ...
    then nil
             ...
             val _ = R.inRangeInt(0, 100, seed)
             val _ = Threads.spawnOn(i-1, fn _ => (C.runClient(i, numTransPerClient, numQPerTrans, qRange, percentUser, manager, seed); PrimChan.send(ch, i)))
         in ch::start (i-1) end
