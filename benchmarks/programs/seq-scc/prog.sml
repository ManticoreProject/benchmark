...
structure GraphLoader = struct
    ...
    let
        ...
        fun cut line = List.map rd (String.tokens Char.isSpace line)
    ...
    *)
    ...
    fun main _ = let
      ...
      fun printListOf pe l = let
        fun printElmsOf ([]) = print "]"
          | printElmsOf (x::xs) = (
              (pe x) ;
              (if List.null xs
              then print "]"
              else (print "," ; printElmsOf xs))
            )
        in
            (print "[" ; printElmsOf l)
        end

      val printer = (printListOf (printListOf (fn x => print (Int.toString x))))

      (* fun withPrint res = (printer res ; print "\n") *)
      ...
      in
          (RunSeq.run start ; OS.Process.success)
      end
