...
type graph = (vertex_id parray * edge_list parray)
...
fun noEarlierNeighbors (pi : vertex_idx parray) 
		       (i : int, (v : vertex_id, edges : edge_list)) : iBool =
  iNot (exists [| isEarlier pi i e | e in edges |])


(**** monomorphic versions of otherwise polymorphic operators  ****)

fun customZip (p,q) = let
  val x = PArray.length p
  val y = PArray.length q
  val z = if x < y then x else y
  in
    PArray.tab (z, (fn i => (p!i,q!i)))
  end

fun customFilterByFlag ((ids,edges) : graph, flags) = let
  val pairs1 = [| (x, flag) | x in ids, flag in flags |]
  val pairs2 = [| (y, flag) | y in edges, flag in flags |]
  val ids' =  [| x | (x, flag) in pairs1 where flag = iTrue |]
  val edges' =  [| y | (y, flag) in pairs2 where flag = iTrue |]
  in
    (ids',edges')
  end
 ...
 *)
fun compressGraph ((ids,edges) : graph, flags : iBool parray) : graph = let
  val newVertexLabels = 
        [| if (flg=iTrue) then i else ~1 | flg in flags, i in enumerate flags |]
  fun newLabel v = newVertexLabels!v
  val (ids',edges') = customFilterByFlag ((ids,edges), flags)
  val edges'' = [| [| newLabel e | e in es where newLabel e <> ~1 |] | es in edges |]
  in
    (ids',edges'')
  end
...
(* TODO *)
fun parallelGreedyMIS ((ids,edges) : graph, pi : vertex_idx parray) : vertex_id parray = 
  if PArray.length ids = 0 then
    (PArray.empty ())
  else let
    val n = PArray.length ids
    (* Let W be the set of vertices in V (where G = (V, E)) with no earlier
     * neighbors (based on pi).
     *)
    val Wflags = [| noEarlierNeighbors pi (i, (ids!j,edges!j)) | i in [| 0 to n-1 |], j in [| 0 to n-1 |] |]
    val (W, Wneighbors) = customFilterByFlag ((ids,edges), Wflags)
    (* Let N be the set of those vertices which are neighbored by a vertex
     * in W. *)
    val Nflags = [| if x then iTrue else iFalse | x in PArray.writeBits (n, PArray.flatten Wneighbors) |]
    val V'flags = [| iNot (iOr(w,n)) | w in Wflags, n in Nflags |]
    (* Remove MIS vertices and their neighbors from the graph *)
    val (ids',edges') = compressGraph ((ids,edges), V'flags)
    in
      PArray.concat (W, parallelGreedyMIS ((ids',edges'), pi))
    end
...
structure Main =
  struct
	...
	let
	    ...
	    val (x,pi) = RunPar.runSilent (fn _ => MIS.mkTest (n, 0.1))
	    fun doit () = MIS.parallelGreedyMIS (customUnzip x, pi)
	in
Print.printLn "doit";
	    RunPar.run doit
	end
