...
# NOTE: don't change these names without updating the loop below!
sub_versions=(
  "$RES/gcstats_footprint"
  "$RES/gcstats"
  "$RES/normal"
)
...
for versionDir in "${sub_versions[@]}"; do
  ...
  # are we in footprint mode? then we only do one subset of programs.
  if [[ $versionDir == *"footprint"* ]]; then
    echo -e "\n\n\t SPAWN FOOTPRINT TEST. \n\n"
    for test in "${footprint_tests[@]}"; do
        pushd "programs/$test"
  ...
  fi # end of footprint conditional
